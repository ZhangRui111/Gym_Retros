...
class DeepQNetwork:
    ...
    def learn(self):
        ...
        batch_memory_arr = np.zeros((self.batch_size, length * 2 + 1 + self.n_actions*2))
        i = 0
        ...
        next_act_index_batch = \
            batch_memory_arr[:, (length + self.n_actions + 1): (length + self.n_actions * 2 + 1)].astype(int)  # a_
        # convert the binary array to an int.
        next_act_index_batch = binary_array_to_int(next_act_index_batch, self.batch_size)
        ...
        selected_q_next = q_target_out[batch_index, next_act_index_batch]
