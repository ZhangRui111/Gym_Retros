...
def restore_parameters(sess, model, hp):
...
def run_stargunner(env, RL, model, saver, load_step, hp):
    ...
    for i_episode in range(hp.MAX_EPISODES):
        ...
        obser_cv_stack = preprocessing_stack(observation, hp)
        ...
        while True:
            ...
            if total_steps % hp.AGENT_HISTORY_LENGTH == 0:
                ...
                action = RL.choose_action(np.expand_dims(preprocessing_stack(observation, hp), axis=0))
